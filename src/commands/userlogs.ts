...
export default new Command('userlogs', async (caller, cmd) => {
	if (!process.env.LOGS_URL)
		return caller.utils.discord.createMessage(cmd.channel.id, 'URL no configurados.');
	if (!cmd.args[0])
		return caller.utils.discord.createMessage(cmd.channel.id, 'ID del usuario?.');
	const links = await caller.db.getUserLogs(cmd.args[0]);
	if (!links) return caller.utils.discord.createMessage(cmd.channel.id, 'Logs no encontrados.');
